/* NovaAI landing-page interactions. Authentication and chat are browser-only demos. */
...
function showToast(message) {
  ...
  showToast.timeoutId = setTimeout(() => toast.classList.remove("is-visible"), 4000);
}

/* ---------- Demo account and browser session ---------- */
...
function updateAuthUI() {
  ...
  accountStatus.textContent = currentUser ? `Hi, ${currentUser.name.split(" ")[0]}` : "";
...
function setAuthView(viewName) {
  ...
  setTimeout(() => modal.querySelector(`.auth-view[data-view="${viewName}"] input`)?.focus(), 0);
}
...
authEntry.addEventListener("click", () => openAuthModal(getDemoUsers().length ? "login" : "register"));
...
document.querySelectorAll("[data-open-auth]").forEach((button) => {
  button.addEventListener("click", () => openAuthModal(getDemoUsers().length ? "login" : "register"));
});
...
document.getElementById("registerForm").addEventListener("submit", async (event) => {
  ...
  if (users.some((user) => user.email === email)) {
    showFormMessage(form, "An account with this email already exists. Please sign in.");
    return;
  }
...
document.getElementById("forgotForm").addEventListener("submit", (event) => {
  ...
  const account = getDemoUsers().find((user) => user.email === email);
  if (!account) {
    showFormMessage(form, "No account was found for this email. You can create another account instead.");
    return;
  }
  const code = String(Math.floor(100000 + Math.random() * 900000));
  localStorage.setItem("novaAIPasswordReset", JSON.stringify({ email, code, expiresAt: Date.now() + 10 * 60 * 1000 }));
  document.getElementById("resetCodeHint").textContent = `Demo verification code: ${code}. This code expires in 10 minutes.`;
  form.reset();
  setAuthView("reset");
...
document.getElementById("resetForm").addEventListener("submit", async (event) => {
  ...
  let request;
  try {
    request = JSON.parse(localStorage.getItem("novaAIPasswordReset") || "null");
  } catch {
    request = null;
  }
  ...
  if (!request || request.expiresAt < Date.now()) {
    ...
    showFormMessage(form, "This verification code has expired. Request a new one.");
    return;
  }
  ...
  const users = getDemoUsers();
  const accountIndex = users.findIndex((user) => user.email === request.email);
  if (accountIndex === -1) {
    showFormMessage(form, "This account is no longer available. Create another account instead.");
    return;
  }
  users[accountIndex].passwordHash = await hashPassword(document.getElementById("resetPassword").value);
  saveDemoUsers(users);
  localStorage.removeItem("novaAIPasswordReset");
  form.reset();
  closeAuthModal();
...
function getPlanCard(plan) {
  return document.querySelector(`[data-plan-card="${plan}"]`) || document.querySelector('[data-plan-card="professional"]');
}
...
function updateCardPreview() {
  ...
  document.getElementById("previewCardNumber").textContent = cardNumber || "4242 4242 4242 4242";
  document.getElementById("previewCardName").textContent = cardName ? cardName.toUpperCase() : "YOUR NAME";
...
function setCheckoutPlan(plan) {
  ...
  const features = [...card.querySelectorAll("li")].slice(0, 3).map((item) => item.textContent.trim());
  ...
  document.getElementById("checkoutPrice").innerHTML = `$${price} <small>${billingPeriod === "yearly" ? "/ month, billed yearly" : "/ month"}</small>`;
  document.getElementById("checkoutFeatures").replaceChildren(...features.map((feature) => {
...
document.getElementById("checkoutForm").addEventListener("submit", (event) => {
  ...
  if (!passesLuhn(document.getElementById("checkoutCard").value)) {
    showFormMessage(form, "Enter a valid demo debit card number. Try 4242 4242 4242 4242.");
    return;
  }
  ...
  form.reset();
  closeCheckout();
  showToast(`${getPlanCard(selectedCheckoutPlan).querySelector("h3").textContent.trim()} trial activated`);
});

/* ---------- Keyboard focus handling for dialogs ---------- */
document.addEventListener("keydown", (event) => {
  if (!modal.classList.contains("is-open")) return;
  if (event.key === "Escape") return closeAuthModal();
  if (event.key !== "Tab") return;
  const focusable = [...dialog.querySelectorAll("button, input, a[href]")].filter((item) => !item.disabled && !item.closest("[hidden]"));
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key !== "Tab") return;
  const focusable = [...checkoutDialog.querySelectorAll("button, input")].filter((item) => !item.disabled);
  const first = focusable[0];
  const last = focusable.at(-1);
  if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); }
  if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); }
...
/* ---------- Pricing and current-section navigation ---------- */
document.querySelectorAll("[data-billing]").forEach((button) => {
  button.addEventListener("click", () => {
    const billingPeriod = button.dataset.billing;
    document.querySelectorAll("[data-billing]").forEach((item) => item.classList.toggle("is-selected", item === button));
    document.querySelectorAll(".price [data-monthly]").forEach((price) => { price.textContent = price.dataset[billingPeriod]; });
    document.querySelectorAll("[data-price-suffix]").forEach((suffix) => { suffix.textContent = billingPeriod === "yearly" ? "/ month, billed yearly" : "/ month"; });
    if (checkoutModal.classList.contains("is-open")) setCheckoutPlan(selectedCheckoutPlan);
  });
});
...
/* ---------- AI assistant: saved local chat history ---------- */
...
const welcomeMessage = "Hi! I am the NovaAI assistant. How can I help you explore the product?";

function getChatHistory() {
  try { return JSON.parse(localStorage.getItem("novaAIChatHistory") || "[]"); } catch { return []; }
}
function saveChatHistory(history) { localStorage.setItem("novaAIChatHistory", JSON.stringify(history)); }
function formatTime(timestamp) { return new Date(timestamp).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }); }
...
function renderChat() {
  ...
  const messages = history.length ? history : [{ role: "bot", text: welcomeMessage, timestamp: Date.now() }];
  messages.forEach((message) => chatMessages.append(createChatMessage(message)));
  chatMessages.scrollTop = chatMessages.scrollHeight;
}
function setChatOpen(isOpen) {
  chatPanel.hidden = !isOpen;
  chatToggle.setAttribute("aria-expanded", String(isOpen));
  chatToggle.setAttribute("aria-label", isOpen ? "Close AI chat assistant" : "Open AI chat assistant");
  if (isOpen) { renderChat(); setTimeout(() => chatInput.focus(), 0); }
}
...
chatForm.addEventListener("submit", (event) => { event.preventDefault(); sendChatMessage(chatInput.value); chatInput.value = ""; });
quickReplies.addEventListener("click", (event) => { if (event.target.matches("button")) sendChatMessage(event.target.textContent); });
clearChat.addEventListener("click", () => { localStorage.removeItem("novaAIChatHistory"); renderChat(); showToast("Chat cleared"); });
